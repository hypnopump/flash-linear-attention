...
def fused_recurrent_bwd_kernel(
    ...
    dalpha,  # gradient of alpha [B, H, L]
    ...
    p_alpha = alpha + i_bh * T + T - 1
    p_dalpha = dalpha + (i_bh + i_v * B * H) * T + T - 1
    ...
    for _ in range(T):
        ...
        _v = tl.load(p_v, mask=mask_bv, other=0).to(tl.float32)
        _beta = tl.load(p_beta).to(tl.float32)
        _alpha = tl.load(p_alpha).to(tl.float32)
        ...
        # TODO: store h or recompute H in this forward bc maybe it's (d_h * h).sum()
        d_alpha = tl.sum(d_h)

        tl.store(p_dk, d_k.to(p_dk.dtype.element_ty), mask=mask_bk)
        tl.store(p_dv, d_v.to(p_dv.dtype.element_ty), mask=mask_bv)
        tl.store(p_dbeta, d_beta.to(p_dbeta.dtype.element_ty))
        tl.store(p_dalpha, d_alpha.to(p_dalpha.dtype.element_ty))

        d_h = d_h * alpha[None, None] - _k[:, None] * d_v[None, :]
    ...
    for i in range(0, T):
        ...
        h = h * _alpha[None, None] + _k[:, None] * _v[None, :]
        _d_q = h * _do[None, :]
...
class FusedRecurrentFunction(torch.autograd.Function):
    ...
    @staticmethod
    @contiguous
    def forward(ctx, q, k, v, beta, alpha, initial_state=None, output_final_state=False):
        ...
        o = o.sum(0)
        ctx.save_for_backward(q, k, v, beta, alpha, initial_state)
    ...
    def backward(ctx, do, d_final_state=None):
        ...
        dalpha = q.new_empty(NV, batch_size, n_heads, seq_len)

        fused_recurrent_bwd_kernel[grid](
            q, k, v, beta, alpha, do, dq, dk, dv, dbeta, dalpha, initial_state,
            q.stride(1), q.stride(2), q.stride(3),
            v.stride(1), v.stride(2), v.stride(3),
            batch_size, n_heads, seq_len, scale,
            DK=d_head_qk, DV=d_head_v, BK=BK, BV=BV,
        ...
        dbeta = dbeta.sum(0)
        dalpha = dalpha.sum(0)
        return dq.to(q), dk.to(k), dv.to(v), dbeta.to(beta), dalpha.to(alpha), None, None
...
def fused_recurrent_linear_attn_delta_rule(
    ...
    if alpha is None:
        alpha = torch.ones_like(q[..., 0])
    o, final_state = FusedRecurrentFunction.apply(q, k, v, beta, alpha, initial_state, output_final_state)
    return o, final_state
...
if __name__ == "__main__":
    ...
    def abakava_fw(q: th.Tensor, k: th.Tensor, v: th.Tensor, w: th.Tensor, beta: th.Tensor, state=None) -> tuple[
        th.Tensor, th.Tensor]:
        """ Inputs:
        * q, k, v, w: (B, H, L, D)
        * beta: (B, H, L)
    ...
    ### compute torch with gradients
    o, s = abakava_fw(q, k, v, w, beta)
    o.retain_grad()
    l = loss(o)
