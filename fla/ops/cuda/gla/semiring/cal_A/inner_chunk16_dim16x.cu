...
std::vector<torch::Tensor> bwd_cuda(torch::Tensor Q,
torch::Tensor K,  
torch::Tensor g_K, torch::Tensor DQK)
 {
    ...
    switch (Q.type().scalarType()) {
        case torch::ScalarType::BFloat16:
                bwd_inner_chunk16_dim16x<<<gridDim, blockDim>>>(B_size * H_size * num_chunk, M, N_K, 
                        Q.data_ptr<bf16>(),
                        K.data_ptr<bf16>(), 
                        g_K.data_ptr<float>(),
                        DQK.data_ptr<bf16>(),
            ...
            break;
        default:
            AT_DISPATCH_FLOATING_TYPES_AND_HALF(Q.scalar_type(), "bwd_inner_chunk16_dim16x", ([&] {
                bwd_inner_chunk16_dim16x<scalar_t><<<gridDim, blockDim>>>(B_size * H_size * num_chunk, M, N_K, 
                        Q.data_ptr<scalar_t>(),
                        K.data_ptr<scalar_t>(), 
                        g_K.data_ptr<float>(),
...
torch::Tensor& K, torch::Tensor& g_K){
    ...
    switch (Q.type().scalarType()) {
        case torch::ScalarType::BFloat16:
                fwd_inner_chunk16_dim16x<bf16><<<gridDim, blockDim>>>(B_size * H_size * num_chunk, M, N_K, 
                        Q.data_ptr<bf16>(),
                        K.data_ptr<bf16>(), 
                        g_K.data_ptr<float>(),
                        QK.data_ptr<bf16>()
            ...
            break;
        default:
      AT_DISPATCH_FLOATING_TYPES_AND_HALF(Q.scalar_type(), "fwd_inner_chunk16_dim16x", ([&] {
          fwd_inner_chunk16_dim16x<scalar_t><<<gridDim, blockDim>>>(B_size * H_size * num_chunk, M, N_K, 
                  Q.data_ptr<scalar_t>(),
                  K.data_ptr<scalar_t>(), 
                  g_K.data_ptr<float>(),
